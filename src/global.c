...
Status GlobalInit(Error *error) {
    if ((GlobalNil = NilCreate(error)) == NULL) goto deallocGlobal;
    if ((GlobalBooleanTrue = BooleanCreate(true, error)) == NULL) goto deallocGlobal;
    if ((GlobalBooleanFalse = BooleanCreate(false, error)) == NULL) goto deallocGlobal;
    if ((GlobalStack = StackCreate(128, error)) == NULL) goto deallocGlobal;
